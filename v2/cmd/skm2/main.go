...
Usage:
  ...
  skm2 route list NODE [--user USER]
  skm2 route add NODE TYPE DESTINATION [--user USER] [--port PORT] [--priority PRIORITY] [--proxy-jump JUMP]
  skm2 route remove NODE ROUTE_ID [--user USER]
...
func main() {
	...
	case "migrate":
		...
		if err != nil { die(err) }
		if len(pos) > 1 { die(errors.New("migrate v1 accepts at most one V1_CONFIG_DIR")) }
		dir := ""
		if len(pos) == 1 { dir = pos[0] }
	...
	case "node":
		...
		case "enroll", "unenroll":
			...
			if err != nil { die(err) }
			if len(pos) != 1 { die(fmt.Errorf("node %s requires exactly one node name", a[1])) }
		...
		case "inspect":
			...
			if err != nil { die(err) }
			if len(pos) != 1 { die(errors.New("node inspect requires exactly one node name")) }
			f := load()
			inspection, enriched, err := remote.Inspect(f, pos[0], opts["--user"])
			if err != nil { die(err) }
			if enriched {
				if err := store.Save(f); err != nil { die(err) }
		...
		case "bridge-version":
			pos, opts, _, err := parseArgs(a[2:], "--user")
			if err != nil { die(err) }
			if len(pos) != 1 { die(errors.New("node bridge-version requires exactly one node name")) }
		...
		case "rollback":
			...
			if err != nil { die(err) }
			if len(pos) != 1 || opts["--expected"] == "" { die(errors.New("node rollback requires NODE and --expected REVISION")) }
	...
	case "route":
		...
		case "list":
			pos, opts, _, err := parseArgs(a[2:], "--user")
			if err != nil { die(err) }
			if len(pos) != 1 { die(errors.New("route list requires NODE")) }
			routes, err := listRoutes(load(), pos[0], opts["--user"])
			if err != nil { die(err) }
			out(routes)
		case "add":
			pos, opts, _, err := parseArgs(a[2:], "--user", "--port", "--priority", "--proxy-jump")
			if err != nil { die(err) }
			if len(pos) != 3 { die(errors.New("route add requires NODE TYPE DESTINATION")) }
			port, err := parsePositiveInt(opts["--port"], "--port", 0)
			if err != nil { die(err) }
			priority, err := parsePositiveInt(opts["--priority"], "--priority", 50)
			if err != nil { die(err) }
			r, err := addRoute(load(), pos[0], opts["--user"], pos[1], pos[2], port, priority, opts["--proxy-jump"])
			if err != nil { die(err) }
			out(r)
		case "remove":
			pos, opts, _, err := parseArgs(a[2:], "--user")
			if err != nil { die(err) }
			if len(pos) != 2 { die(errors.New("route remove requires NODE ROUTE_ID")) }
			if err := removeRoute(load(), pos[0], opts["--user"], pos[1]); err != nil { die(err) }
	...
	case "host":
		...
		case "scan", "verify":
			pos, opts, _, err := parseArgs(a[2:], "--user")
			if err != nil { die(err) }
			if len(pos) != 1 { die(fmt.Errorf("host %s requires exactly one node name", a[1])) }
			if a[1] == "scan" {
		...
		case "trust":
			...
			if err != nil { die(err) }
			if len(pos) != 1 || opts["--key"] == "" { die(errors.New("host trust requires NODE --key FILE")) }
		...
		case "rotate":
			...
			if err != nil { die(err) }
			if len(pos) != 1 || opts["--key"] == "" || opts["--expected-old"] == "" { die(errors.New("host rotate requires NODE --expected-old FINGERPRINT --key FILE")) }
	...
	case "subject":
		...
		case "list":
			out(load().Subjects)
		case "add":
			if len(a) < 3 || len(a) > 4 { die(errors.New("subject add requires NAME [TYPE]")) }
			kind := "device"
			if len(a) == 4 { kind = a[3] }
	...
	case "credential":
		...
		case "import":
			if len(a) != 4 { die(errors.New("credential import requires SUBJECT PUBLIC_KEY_FILE")) }
			c, err := importCredential(load(), a[2], a[3])
			if err != nil { die(err) }
			out(c)
		case "rotate":
			if len(a) != 4 { die(errors.New("credential rotate requires SUBJECT PUBLIC_KEY_FILE")) }
			r, err := rotateCredential(load(), a[2], a[3])
			if err != nil { die(err) }
			out(r)
		case "retire", "activate":
			if len(a) != 4 { die(fmt.Errorf("credential %s requires SUBJECT CREDENTIAL", a[1])) }
			status := model.CredentialRetired
			if a[1] == "activate" { status = model.CredentialActive }
	...
	case "policy":
		...
		case "grant", "revoke":
			pos, opts, _, err := parseArgs(a[2:], "--user")
			if err != nil { die(err) }
			if len(pos) != 2 { die(fmt.Errorf("policy %s requires SUBJECT NODE", a[1])) }
			f := load()
			if a[1] == "grant" {
				...
				if err := revokePolicy(f, pos[0], pos[1], opts["--user"]); err != nil { die(err) }
				fmt.Println("policy grant removed")
			}
		case "mode":
			pos, opts, _, err := parseArgs(a[2:], "--user")
			if err != nil { die(err) }
			if len(pos) != 2 { die(errors.New("policy mode requires NODE MODE")) }
	...
	case "plan":
		...
		if err != nil { die(err) }
		if len(pos) != 0 { die(fmt.Errorf("unexpected positional plan arguments: %s", strings.Join(pos, " "))) }
		if (opts["--node"] == "") == (opts["--observed"] == "") { die(errors.New("plan requires exactly one of --node NAME or --observed FILE")) }
		f := load()
		...
		if opts["--node"] != "" {
			...
			if err != nil { die(err) }
			if err := json.Unmarshal(b, &observed); err != nil { die(err) }
		}
		plan := planner.Build(f, observed)
		if opts["--out"] != "" {
			if err := writeJSONFile(opts["--out"], plan); err != nil { die(err) }
	...
	case "apply":
		...
		if err != nil { die(err) }
		if len(pos) != 1 { die(errors.New("apply requires exactly one PLAN_FILE")) }
		requireYes(flags, "apply")
		p, err := readPlan(pos[0])
		if err != nil { die(err) }
		results, err := remote.ApplyPlanSafe(load(), p)
		if err != nil { die(err) }
		out(results)

	case "backup":
		if len(a) < 3 { usage(); os.Exit(2) }
		switch a[1] {
		case "create":
			if len(a) != 3 { die(errors.New("backup create requires FILE")) }
			if err := store.Backup(a[2], load()); err != nil { die(err) }
			fmt.Println(filepath.Clean(a[2]))
		case "restore":
			pos, _, flags, err := parseArgs(a[2:])
			if err != nil { die(err) }
			if len(pos) != 1 { die(errors.New("backup restore requires FILE")) }
